#!/usr/bin/env node
// Runs after `npm install -g @scaledown/claude-plugin`. Refreshes managed config
...
// `scaledown-claude setup` does first-time configuration.
...
try {
  ...
  const touched = Object.entries(result)
    ...
    .map(([k]) => k);
  if (touched.length > 0) {
    console.log(`scaledown: refreshed config for ${touched.join(", ")}`);
  }
} catch {
